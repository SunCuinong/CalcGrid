...
def save_words_and_push(data):
    ...
    try:
        subprocess.run(["git", "add", "dictation/data/words.json"],
                       cwd=os.path.dirname(BASE_DIR), check=True)
        subprocess.run(["git", "commit", "-m", "chore(dictation): sync words data"],
                       cwd=os.path.dirname(BASE_DIR), check=True,
                       stderr=subprocess.DEVNULL)
        # 推送到所有可推送的远程（origin 优先，其次 github）
